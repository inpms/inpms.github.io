...
P-tuning虽然是一种peft方法，但并不能于huggingface的peft python包兼容，而fastchat在多处以字符串匹配的方式进行硬编码加载模型，因此导致fastchat和chatchat不能兼容p-tuning，经ecap开发组多次尝试，给出如下指南进行p-tuning加载。
...
至此，我们完成了ecap加载p-tuning的全部操作，将ptuing的路径添加到model_config的llm_dict，如
```
...
![成功上传知识库](https://github.com/chatchat-space/ecap/blob/dev/img/init_knowledge_base.jpg)
...
![LLM对话](https://github.com/chatchat-space/ecap/blob/dev/img/LLM_success.png)
...
![成功调用知识库](https://github.com/chatchat-space/ecap/blob/dev/img/knowledge_base_success.jpg)
...
![成功调用单个Agent工具](https://github.com/chatchat-space/ecap/blob/dev/img/agent_success.png)

本框架支持模型连续掉用多个Agent工具，下图展示了一个一个提问中大模型连续调用多个Agent工具的效果图:

![连续调用多个Agent工具](https://github.com/chatchat-space/ecap/blob/dev/img/agent_continue.png)
